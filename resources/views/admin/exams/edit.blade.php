...
$workspaceConfig = [
    ...
    'examName' => (string)old('exam_name', $exam->exam_name),
    'headerTitle' => (string)old('header_title', $exam->header_title ?? ''),
    'vendorId' => (string)old('vendor_id', $exam->vendor_id),
    'vendorName' => (string)($exam->vendor ? $exam->vendor->name : 'Unknown Vendor'),
    'vendorSlug' => (string)($exam->vendor ? $exam->vendor->slug : ''),
    'difficulty' => (string)old('difficulty', $exam->difficulty),
    'examType' => (string)old('exam_type', $exam->exam_type),
    'passingScore' => (int)old('passing_score', $exam->passing_score),
    'questionCount' => (string)old('question_count', (string)$exam->question_count),
    'actualQuestions' => (int)$actualQuestionCount,
    'isPdfAvailable' => (bool)old('is_pdf_available', $exam->is_pdf_available ?? true),
    'isEngineAvailable' => (bool)old('is_engine_available', $exam->is_engine_available ?? true),
    'isBundleAvailable' => (bool)old('is_bundle_available', $exam->is_bundle_available ?? true),
    'pricePdf' => (string)old('price_pdf', $exam->price_pdf),
    'priceEngine' => (string)old('price_engine', $exam->price_engine),
    'priceBundle' => (string)old('price_bundle', $exam->price_bundle ?? ''),
    'update3' => (string)old('update_price_3_months', $exam->update_price_3_months),
    'update6' => (string)old('update_price_6_months', $exam->update_price_6_months),
    'update12' => (string)old('update_price_12_months', $exam->update_price_12_months),
    'isActive' => (bool)old('is_active', $exam->is_active),
    'isFeatured' => (bool)old('is_featured', $exam->is_featured),
    'slug' => (string)old('slug', $exam->slug),
    'sortOrder' => (int)old('sort_order', $exam->sort_order),
    'metaTitle' => (string)old('meta_title', $exam->meta_title ?? ''),
    'metaDescription' => (string)old('meta_description', $exam->meta_description ?? ''),
    'metaKeywords' => (string)old('meta_keywords', $exam->meta_keywords ?? ''),
...
<div x-data="examWorkspace({{ json_encode($workspaceConfig) }})" class="space-y-8 pb-20">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            ...
            <div class="space-y-2">
                <div class="flex items-center space-x-2 text-xs font-semibold uppercase tracking-wider text-gray-500">
                    ...
                    <span>/</span>
                    <span class="text-navy font-bold">Edit Exam</span>
                </div>
                <div class="flex flex-wrap items-center gap-3">
                    <h1 class="text-2xl sm:text-3xl font-black text-navy tracking-tight font-heading" x-text="examCode || 'Edit Exam'">
                        {{ $exam->exam_code }}
                    </h1>
                    <span class="text-gray-300 text-xl font-light">|</span>
                    <span class="text-lg sm:text-xl text-gray-700 font-medium truncate max-w-md" x-text="examName || 'Exam Name'">
                        {{ $exam->exam_name }}
                    </span>
                    <!-- Status Badge -->
                    ...
                    @if($exam->is_featured)
                        <span class="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-extrabold bg-purple-100 text-purple-700 border border-purple-200 uppercase tracking-wider">
                            ★ Featured
                        </span>
                    @endif
                </div>
                <p class="text-xs text-gray-500 flex items-center gap-3">
                    <span>Vendor: <strong class="text-gray-700 font-semibold">{{ $exam->vendor->name ?? 'N/A' }}</strong></span>
                    <span>•</span>
                    <span>Last Updated: <strong class="text-gray-700 font-semibold">{{ $exam->updated_at ? $exam->updated_at->format('M d, Y h:i A') : 'N/A' }}</strong></span>
                </p>
            </div>

            <!-- Right: Primary Actions -->
            <div class="flex items-center space-x-3 shrink-0">
                <a href="{{ $exam->url }}" target="_blank" class="inline-flex items-center px-4 py-2.5 rounded-lg border border-gray-300 bg-white text-gray-700 hover:bg-gray-50 text-xs font-bold transition shadow-sm">
                    <svg class="w-4 h-4 mr-1.5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
                    Preview Public Page
                </a>
    ...
    <form id="examForm" action="{{ route('admin.exams.update', $exam->id) }}" method="POST" enctype="multipart/form-data" @submit="isSubmitting = true">
        @csrf
        @method('PUT')
                            ...
                            <div>
                                ...
                                <select name="vendor_id" id="vendor_id" required x-model="vendorId" @change="onVendorChange($event)"
                                    ...
                                    @foreach($vendors as $vendor)
                                        <option value="{{ $vendor->id }}" data-name="{{ $vendor->name }}" data-slug="{{ $vendor->slug }}" {{ old('vendor_id', $exam->vendor_id) == $vendor->id ? 'selected' : '' }}>
                                            {{ $vendor->name }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 pt-2">
                            ...
                            <div>
                                ...
                                <input type="number" name="question_count" id="question_count" x-model="questionCount" @input="markDirty()" min="0" placeholder="Auto or manual"
                                ...
                                <p class="text-[10px] text-gray-400 mt-1">
                                    Real DB: <strong class="text-navy font-bold">{{ $actualQuestionCount }}</strong> questions.
                                </p>
                                @error('question_count') <p class="text-red-500 text-xs mt-1.5 font-medium">{{ $message }}</p> @enderror
                            </div>
                        </div>

                        <!-- Real Database Questions Notification Card -->
                        <div class="bg-gray-50 border border-gray-200 rounded-lg p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                            <div class="flex items-center space-x-3">
                                <div class="w-9 h-9 rounded-lg bg-navy/5 flex items-center justify-center text-navy shrink-0">
                                    <svg class="w-5 h-5 text-cyan" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                </div>
                                <div>
                                    <div class="text-xs font-bold text-navy">
                                        Database Questions: <span class="text-cyan font-extrabold">{{ $actualQuestionCount }} Verified Questions</span>
                                    </div>
                                    <div class="text-[11px] text-gray-500">
                                        {{ $actualQuestionCount > 0 ? 'Questions are actively linked to this exam and available for the Simulator.' : 'No questions assigned yet in the test bank.' }}
                                    </div>
                                </div>
                            </div>
                            <div class="flex items-center gap-2">
                                <a href="{{ route('admin.questions.index', ['exam_id' => $exam->id]) }}" class="text-xs font-bold text-navy hover:text-cyan bg-white border border-gray-300 hover:border-cyan px-3 py-1.5 rounded-md transition shadow-sm inline-flex items-center">
                                    Manage Questions →
                                </a>
                                <a href="{{ route('admin.questions.create', ['exam_id' => $exam->id]) }}" class="text-xs font-bold text-white bg-navy hover:bg-gray-800 px-3 py-1.5 rounded-md transition shadow-sm inline-flex items-center">
                                    + Add
                                </a>
                            </div>
                        </div>
                ...
                <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden" id="section-article">
                    ...
                    <div class="p-6 space-y-4">
                        @include('admin.partials.tiptap-editor', [
                            ...
                            'value' => old('article_content', $exam->article_content ?? ''),
                    ...
                    <div class="p-6 space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="p-5 border border-gray-200 rounded-xl bg-gray-50/40 space-y-3">
                                ...
                                <!-- Existing File Chip if uploaded -->
                                @if($exam->demo_pdf_filename)
                                    <div class="bg-white border border-gray-200 rounded-lg p-3 space-y-2">
                                        <div class="flex items-start justify-between">
                                            <div class="flex items-center space-x-2.5">
                                                <div class="w-8 h-8 rounded bg-red-100 text-red-600 flex items-center justify-center shrink-0 font-bold text-xs">PDF</div>
                                                <div class="truncate max-w-[180px]">
                                                    <div class="text-xs font-bold text-navy truncate" title="{{ $exam->demo_pdf_filename }}">{{ $exam->demo_pdf_filename }}</div>
                                                    <div class="text-[10px] text-gray-400">
                                                        {{ $demoFileInfo['size_formatted'] ?? 'Available' }} • {{ $demoFileInfo['last_modified'] ?? 'Uploaded' }}
                                                    </div>
                                                </div>
                                            </div>
                                            <a href="{{ route('admin.exams.download-pdf', [$exam->id, 'demo']) }}" target="_blank" class="text-xs font-bold text-cyan hover:underline inline-flex items-center">
                                                Preview ↗
                                            </a>
                                        </div>
                                        ...
                                        <div class="pt-2 border-t border-gray-100 flex items-center justify-between">
                                            <label class="flex items-center text-xs text-red-600 hover:text-red-700 cursor-pointer font-medium">
                                                <input type="checkbox" name="remove_demo_pdf" value="1" class="rounded border-gray-300 text-red-600 focus:ring-red-500 mr-2 h-3.5 w-3.5">
                                                Remove current demo PDF
                                            </label>
                                        </div>
                                    </div>
                                @endif
                                ...
                                <div>
                                    <label class="block text-[11px] font-semibold text-gray-500 mb-1">
                                        {{ $exam->demo_pdf_filename ? 'Replace with new PDF guide:' : 'Upload Demo PDF Guide:' }}
                                    </label>
                            ...
                            <div class="p-5 border border-gray-200 rounded-xl bg-gray-50/40 space-y-3">
                                    ...
                                    <div class="bg-white border border-gray-200 rounded-lg p-3 space-y-2">
                                        <div class="flex items-start justify-between">
                                            <div class="flex items-center space-x-2.5">
                                                <div class="w-8 h-8 rounded bg-red-100 text-red-600 flex items-center justify-center shrink-0 font-bold text-xs">PDF</div>
                                                <div class="truncate max-w-[180px]">
                                                    <div class="text-xs font-bold text-navy truncate" title="{{ $exam->full_pdf_filename }}">{{ $exam->full_pdf_filename }}</div>
                                                    <div class="text-[10px] text-gray-400">
                                                        {{ $fullFileInfo['size_formatted'] ?? 'Available' }} • {{ $fullFileInfo['last_modified'] ?? 'Uploaded' }}
                                                    </div>
                                                </div>
                                            </div>
                                            <a href="{{ route('admin.exams.download-pdf', [$exam->id, 'full']) }}" target="_blank" class="text-xs font-bold text-cyan hover:underline inline-flex items-center">
                                                Download ↗
                                            </a>
                                        </div>
                                        ...
                                        <div class="pt-2 border-t border-gray-100 flex items-center justify-between">
                                            <label class="flex items-center text-xs text-red-600 hover:text-red-700 cursor-pointer font-medium">
                                                <input type="checkbox" name="remove_full_pdf" value="1" class="rounded border-gray-300 text-red-600 focus:ring-red-500 mr-2 h-3.5 w-3.5">
                                                Remove current full PDF
                                            </label>
                                        </div>
                                    </div>
                                @endif
                                ...
                                <div>
                                    <label class="block text-[11px] font-semibold text-gray-500 mb-1">
                                        {{ $exam->full_pdf_filename ? 'Replace with new PDF guide:' : 'Upload Complete PDF Guide:' }}
                                    </label>
                    ...
                    <div x-show="advancedOpen" class="p-6 space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <p class="text-[11px] text-amber-600 mt-1 font-medium flex items-center gap-1">
                                    ...
                                    Changing existing slug automatically creates a permanent 301 redirect.
                                </p>
                        ...
                        <div>
                            ...
                            <textarea name="admin_notes" id="admin_notes" rows="3" placeholder="Add confidential notes regarding question pool updates, source material, or vendor revision dates..."
                                      class="w-full border-gray-300 rounded-lg text-sm px-3.5 py-2.5 focus:border-cyan focus:ring-cyan shadow-sm">{{ old('admin_notes', $exam->admin_notes) }}</textarea>
            ...
            <div class="lg:col-span-4 space-y-6 lg:sticky lg:top-6">
                ...
                <div class="bg-white border border-gray-200 rounded-xl p-6 shadow-sm space-y-5">
                    ...
                    <div class="space-y-2 pt-2">
                        <button type="submit" @click="document.getElementById('formActionInput').value = 'publish'"
                            ...
                            <svg class="w-4 h-4 text-cyan" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7" /></svg>
                    ...
                    <div class="pt-3 border-t border-gray-100 text-[11px] text-gray-400 flex items-center justify-between">
                        <span>Updated: {{ $exam->updated_at ? $exam->updated_at->diffForHumans() : 'Just now' }}</span>
                        <a href="{{ $exam->url }}" target="_blank" class="text-navy hover:text-cyan font-bold inline-flex items-center">
                            Live Page ↗
                        </a>
                    </div>
                </div>
                    ...
                    <div class="space-y-3 text-xs">
                        ...
                        <div class="flex items-center justify-between py-1 border-b border-gray-50">
                            ...
                            <span class="font-bold text-navy">
                                <span x-text="actualQuestions"></span> DB / <span x-text="questionCount || actualQuestions"></span> Pool
                            </span>
                        </div>
                        <div class="flex items-center justify-between py-1 border-b border-gray-50">
                ...
                <div class="bg-white border border-gray-200 rounded-xl p-6 shadow-sm space-y-4">
                    ...
                    <div class="space-y-1.5 pt-1 text-[11px]">
                        ...
                        <div class="flex items-center justify-between" :class="((isPdfAvailable && pricePdf > 0) || (isEngineAvailable && priceEngine > 0) || (isBundleAvailable && priceBundle > 0)) ? 'text-emerald-700' : 'text-gray-400'">
                            <span>Pricing Configured</span>
                            <span x-text="((isPdfAvailable && pricePdf > 0) || (isEngineAvailable && priceEngine > 0) || (isBundleAvailable && priceBundle > 0)) ? '✓' : '○'"></span>
                        </div>
                        ...
                        <div class="flex items-center justify-between" :class="actualQuestions > 0 ? 'text-emerald-700' : 'text-gray-400'">
                            <span>Questions Assigned in DB</span>
                            <span x-text="actualQuestions > 0 ? '✓' : '○'"></span>
                        </div>
                        <div class="flex items-center justify-between" :class="hasDemoPdf ? 'text-emerald-700' : 'text-gray-400'">
                            <span>Free Demo PDF Attached</span>
                            <span x-text="hasDemoPdf ? '✓' : '○'"></span>
                        </div>
                        <div class="flex items-center justify-between" :class="hasFullPdf ? 'text-emerald-700' : 'text-gray-400'">
                            <span>Full Study Guide PDF Attached</span>
                            <span x-text="hasFullPdf ? '✓' : '○'"></span>
                        </div>
                ...
                <!-- Danger Zone / Delete Exam Safeguard Card -->
                <div class="bg-red-50/50 border border-red-200 rounded-xl p-6 shadow-sm space-y-3" x-data="{ confirmDelete: false }">
                    <h3 class="text-xs font-black text-red-800 uppercase tracking-wider">Danger Zone</h3>
                    <p class="text-xs text-red-600 leading-relaxed">
                        Deleting an exam is permanent. Exams with active customer purchases or order items are protected and cannot be deleted.
                    </p>

                    <div x-show="!confirmDelete">
                        <button type="button" @click="confirmDelete = true" class="w-full bg-white border border-red-300 hover:bg-red-600 hover:text-white text-red-600 font-bold py-2 px-4 rounded-lg text-xs transition shadow-sm">
                            Delete Exam...
                        </button>
                    </div>

                    <div x-show="confirmDelete" class="space-y-2 pt-2" style="display: none;">
                        <p class="text-[11px] font-bold text-red-700">Are you absolutely sure? This will delete {{ $exam->exam_code }}.</p>
                        <div class="flex gap-2">
                            <button type="button" @click="confirmDelete = false" class="flex-1 bg-white border border-gray-300 text-gray-700 font-bold py-1.5 px-3 rounded text-xs hover:bg-gray-100">
                                Cancel
                            </button>
                            <button type="button" onclick="document.getElementById('deleteExamForm').submit()" class="flex-1 bg-red-600 text-white font-bold py-1.5 px-3 rounded text-xs hover:bg-red-700">
                                Confirm Delete
                            </button>
                        </div>
                    </div>
    ...
    <!-- Hidden Delete Form -->
    <form id="deleteExamForm" action="{{ route('admin.exams.destroy', $exam->id) }}" method="POST" class="hidden">
        @csrf
        @method('DELETE')
    </form>
...
function examWorkspace(initial) {
    return {
        ...
        init() {
            ...
            this.$nextTick(() => {
                ...
                if (el && el.selectedIndex > 0) {
                    this.vendorName = el.options[el.selectedIndex].getAttribute('data-name') || this.vendorName;
                    this.vendorSlug = el.options[el.selectedIndex].getAttribute('data-slug') || this.vendorSlug;
                }
            });
        },
        ...
        get completenessScore() {
            let score = 0;
            if (this.vendorId) score += 15;
            if (this.examCode && this.examCode.trim().length > 0) score += 15;
            if (this.examName && this.examName.trim().length > 0) score += 15;
            ...
            if (hasPricing) score += 20;
            if (parseInt(this.passingScore) > 0) score += 10;
            if (this.actualQuestions > 0) score += 10;
            if (this.hasDemoPdf) score += 5;
            if (this.hasFullPdf) score += 5;
            if (this.metaTitle && this.metaDescription) score += 5;
