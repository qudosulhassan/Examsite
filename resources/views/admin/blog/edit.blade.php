...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-lg border border-gray-250 p-6 shadow-sm">
        <form action="{{ route('admin.blog.update', $post->id) }}" method="POST" id="postForm" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="title" id="title" value="{{ old('title', $post->title) }}" required class="w-full text-sm border-gray-300 rounded focus:border-cyan focus:ring-cyan">
            </div>

            <!-- Excerpt -->
            <div>
                <label for="excerpt" class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-2">Excerpt (Brief Summary)</label>
                <textarea name="excerpt" id="excerpt" rows="2" class="w-full text-sm border-gray-300 rounded focus:border-cyan focus:ring-cyan">{{ old('excerpt', $post->excerpt) }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="featured_image" id="featured_image" value="{{ old('featured_image', $post->featured_image) }}" class="w-full text-sm border-gray-300 rounded focus:border-cyan focus:ring-cyan">
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="meta_description" id="meta_description" value="{{ old('meta_description', $post->meta_description) }}" class="w-full text-sm border-gray-300 rounded focus:border-cyan focus:ring-cyan">
                </div>
            </div>
            ...
            <div class="mt-4">
                ...
                <input type="text" name="meta_keywords" id="meta_keywords" value="{{ old('meta_keywords', $post->meta_keywords) }}" placeholder="e.g., tech news, exam tips, certification guide" class="w-full text-sm border-gray-300 rounded focus:border-cyan focus:ring-cyan">
            </div>
