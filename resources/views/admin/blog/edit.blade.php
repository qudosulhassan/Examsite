...
<div class="max-w-5xl mx-auto space-y-6">
    ...
    <form action="{{ route('admin.blog.update', $post->id) }}" method="POST" enctype="multipart/form-data" id="postForm" class="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title', $post->title) }}" required class="w-full text-base border-gray-300 rounded px-4 py-3 focus:border-cyan focus:ring-cyan transition-colors" placeholder="Enter an engaging title...">
                        @error('title')
                    ...
                    <div>
                        ...
                        <input type="text" name="slug" id="slug" value="{{ old('slug', $post->slug) }}" class="w-full text-sm font-mono border-gray-300 rounded px-4 py-3 focus:border-cyan focus:ring-cyan" placeholder="custom-url-slug">
                    ...
                    <div>
                        ...
                        <textarea name="excerpt" rows="3" class="w-full text-sm border-gray-300 rounded px-4 py-3 focus:border-cyan focus:ring-cyan transition-colors" placeholder="A short summary for blog lists and SEO...">{{ old('excerpt', $post->excerpt) }}</textarea>
                    </div>
                </div>
                ...
                <div class="space-y-6 bg-gray-50 p-6 rounded-lg border border-gray-100">
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="published_at" id="published_at" value="{{ old('published_at', $post->published_at ? \Carbon\Carbon::parse($post->published_at)->format('Y-m-d\TH:i') : '') }}" class="w-full text-sm border-gray-300 rounded focus:border-cyan focus:ring-cyan">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <select name="related_exam_id" id="related_exam_id" class="w-full text-sm border-gray-300 rounded focus:border-cyan focus:ring-cyan">
                            ...
                            @foreach($exams as $exam)
                                <option value="{{ $exam->id }}" {{ old('related_exam_id', $post->related_exam_id) == $exam->id ? 'selected' : '' }}>{{ $exam->exam_code }} - {{ $exam->title }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md bg-white hover:bg-gray-50 transition relative overflow-hidden" id="image-drop-area">
                            ...
                            <div class="space-y-1 text-center hidden" id="upload-prompt">
                            @else
                            ...
                            @if($post->featured_image)
                            <img id="image-preview" src="{{ Storage::url($post->featured_image) }}" class="absolute inset-0 w-full h-full object-cover">
                            @else
                    ...
                    <div class="flex items-center pt-2">
                        <input id="is_featured" name="is_featured" type="checkbox" value="1" {{ old('is_featured', $post->is_featured) ? 'checked' : '' }} class="h-4 w-4 text-cyan focus:ring-cyan border-gray-300 rounded">
            ...
            <div class="border-t border-gray-100 pt-8 mt-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="meta_title" id="meta_title" value="{{ old('meta_title', $post->meta_title) }}" class="w-full text-sm border-gray-300 rounded focus:border-cyan focus:ring-cyan">
                    </div>
                    <div>
                        <label for="meta_keywords" class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Meta Keywords</label>
                        <input type="text" name="meta_keywords" id="meta_keywords" value="{{ old('meta_keywords', $post->meta_keywords) }}" placeholder="tech, news, exams" class="w-full text-sm border-gray-300 rounded focus:border-cyan focus:ring-cyan">
                    </div>
                    <div class="md:col-span-2">
                        ...
                        <textarea name="meta_description" id="meta_description" rows="2" class="w-full text-sm border-gray-300 rounded focus:border-cyan focus:ring-cyan">{{ old('meta_description', $post->meta_description) }}</textarea>
                    </div>
                </div>
            </div>
        </div>
