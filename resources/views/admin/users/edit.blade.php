...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div class="flex items-center space-x-3">
            ...
            <form action="{{ route('admin.users.destroy', $user->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure you want to permanently delete user \'{{ addslashes($user->name) }}\' ({{ addslashes($user->email) }})? This will completely vanish the user from the database and cannot be undone.');">
                @csrf
                @method('DELETE')
                <button type="submit" class="inline-flex items-center space-x-1.5 text-xs font-bold text-red-600 bg-red-50 hover:bg-red-100 border border-red-200 px-4 py-2.5 rounded-lg transition shadow-sm">
    ...
    <div class="bg-white border border-gray-250 rounded-xl shadow-sm overflow-hidden">
        ...
        <form action="{{ route('admin.users.update', $user->id) }}" method="POST" enctype="multipart/form-data" class="p-6 sm:p-8 space-y-6">
            ...
            <!-- User Biography / Author Bio -->
            <div class="pt-2">
                <label for="bio" class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-2">User Biography / Author Bio</label>
                <textarea name="bio" id="bio" rows="3" placeholder="Brief professional biography, qualifications, or author introduction (displayed on blog articles and author profile)..."
                          class="w-full text-sm border-gray-250 rounded-lg px-3.5 py-2.5 focus:border-cyan focus:ring-cyan transition">{{ old('bio', $user->bio) }}</textarea>
                <p class="text-[11px] text-gray-400 mt-1.5">Optional. Displayed publicly as the author bio on blog posts and author profile pages.</p>
                @error('bio')
                    <p class="text-red-500 text-xs mt-1 font-semibold">{{ $message }}</p>
                @enderror
            </div>
