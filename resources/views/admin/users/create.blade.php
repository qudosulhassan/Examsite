        ...
        <form action="{{ route('admin.users.store') }}" method="POST" enctype="multipart/form-data" class="p-6 sm:p-8 space-y-6">
            ...
            <div class="pt-2">
                ...
                <textarea name="bio" id="bio" rows="3" placeholder="Brief professional biography, qualifications, or author introduction (displayed on blog articles and author profile)..."
                          class="w-full text-sm border-gray-250 rounded-lg px-3.5 py-2.5 focus:border-cyan focus:ring-cyan transition">{{ old('bio') }}</textarea>
                <p class="text-[11px] text-gray-400 mt-1.5">Optional. Displayed as the author bio on blog posts and author profile pages.</p>
                @error('bio')
                    <p class="text-red-500 text-xs mt-1 font-semibold">{{ $message }}</p>
                @enderror
            </div>
