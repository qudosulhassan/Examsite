                    ...
                    <div class="space-y-1">
                        ...
                        <div class="relative h-48 w-full flex">
                            ...
                            <div class="w-12 sm:w-14 flex-shrink-0 h-full flex flex-col justify-between text-[10px] font-bold text-gray-400 pb-6 pr-2 text-right select-none">
                                <span x-text="chartMetric === 'revenue' ? '${{ number_format($chartData['maxRevenue']) }}' : '{{ $chartData['maxOrders'] }}'"></span>
                                <span x-text="chartMetric === 'revenue' ? '${{ number_format($chartData['maxRevenue'] / 2) }}' : '{{ (int)ceil($chartData['maxOrders'] / 2) }}'"></span>
                                <span>0</span>
                            </div>
                                ...
                                <div class="relative h-full flex items-end gap-1 sm:gap-1.5 pb-6 overflow-x-auto">
                                    ...
                                    @foreach($chartData['points'] as $idx => $pt)
                                        @php
                                            $hasRev = $pt['revenue'] > 0;
                                            $hasOrd = $pt['orders'] > 0;
                                            $revHeight = ($chartData['maxRevenue'] > 0 && $hasRev) ? max(6, min(100, round(($pt['revenue'] / $chartData['maxRevenue']) * 100))) : 0;
                                            $ordHeight = ($chartData['maxOrders'] > 0 && $hasOrd) ? max(6, min(100, round(($pt['orders'] / $chartData['maxOrders']) * 100))) : 0;
                                        @endphp
                                        <div class="flex-1 min-w-[20px] max-w-[48px] h-full flex flex-col items-center justify-end relative group cursor-pointer">
                                            ...
                                            <div class="w-full relative flex flex-col justify-end items-center transition-all duration-300 z-10"
                                                ...
                                                @if($hasRev || $hasOrd)
                                                    <div class="w-full h-full rounded-t-md transition-all duration-300 shadow-sm"
                                                         :class="{
                                                             'bg-gradient-to-t from-cyan-600 via-cyan-500 to-cyan-400 group-hover:from-cyan-500 group-hover:to-cyan-300 shadow-cyan-500/25': chartMetric === 'revenue' && {{ $hasRev ? 'true' : 'false' }},
                                                             'bg-gradient-to-t from-indigo-600 via-indigo-500 to-blue-400 group-hover:from-indigo-500 group-hover:to-blue-300 shadow-indigo-500/25': chartMetric === 'orders' && {{ $hasOrd ? 'true' : 'false' }},
                                                             'bg-transparent': (chartMetric === 'revenue' && !{{ $hasRev ? 'true' : 'false' }}) || (chartMetric === 'orders' && !{{ $hasOrd ? 'true' : 'false' }})
                                                         }">
                                                        <!-- Top edge highlight -->
