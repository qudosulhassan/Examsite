@php
    ...
    $fieldValue = $value ?? '';
    $editorMinHeight = $minHeight ?? '480px';
    $editorPlaceholder = $placeholder ?? 'Draft your comprehensive, high-ranking IT certification guide or article...';
@endphp

<div class="tiptap-container tiptap-full-editor bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden"
     data-editor-type="full"
     data-content="{{ base64_encode($fieldValue) }}"
    ...
    <div class="tiptap-toolbar p-2 bg-slate-50 border-b border-gray-200 flex flex-wrap items-center gap-1 text-xs">
        ...
        <div class="flex items-center gap-0.5 ml-auto">
            ...
            <button type="button" class="btn-find-replace p-1.5 rounded text-gray-700 hover:bg-gray-200 transition" title="Find and Replace in Content">
    ...
    <div class="editor-element w-full bg-white min-h-[{{ $editorMinHeight }}] focus:outline-none"></div>

    <!-- HTML / SOURCE CODE EDITOR ELEMENT (Two-way sync) -->
    <textarea class="source-editor-element hidden w-full min-h-[{{ $editorMinHeight }}] p-4 font-mono text-xs text-slate-100 bg-slate-950 focus:outline-none border-none resize-y leading-relaxed" spellcheck="false"></textarea>

    <!-- HIDDEN FORM INPUT FOR LARAVEL POST DATA -->
    <input type="hidden" name="{{ $fieldName }}" class="content-input" value="{{ $fieldValue }}">
        ...
        <div class="bg-white rounded-xl shadow-2xl max-w-md w-full p-5 space-y-4">
            ...
            <div class="space-y-3 text-xs">
                ...
                <div class="flex items-center gap-2 pt-1">
                    <input type="checkbox" id="link_target_blank_{{ $fieldName }}" class="link-modal-target rounded border-gray-300 text-cyan focus:ring-cyan" checked>
                    <label for="link_target_blank_{{ $fieldName }}" class="text-xs text-gray-700 font-medium cursor-pointer">Open link in new browser tab (<code>target="_blank"</code>)</label>
                </div>
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] flex flex-col overflow-hidden">
            <div class="px-6 py-4 bg-slate-900 text-white flex items-center justify-between border-b border-slate-800">
                <div class="flex items-center gap-2">
                    ...
                    <h3 class="text-sm font-bold tracking-tight">Article Content Live Preview</h3>
