...
<div class="max-w-4xl mx-auto space-y-6"
    ...
    <div class="flex justify-between items-center">
        ...
        <a href="{{ route('admin.questions.index', ['exam_id' => request('exam_id')]) }}" class="text-sm text-navy hover:underline">
            ← Back to Listing
        </a>
    </div>
    ...
    <div class="bg-white border border-gray-250 rounded-lg p-6 shadow-sm">
        <form action="{{ route('admin.questions.store') }}" method="POST" enctype="multipart/form-data" @submit="submitForm($event)" class="space-y-8">
            ...
            <div class="border-b border-gray-150 pb-6">
                ...
                <div class="space-y-3">
                    <template x-for="(ref, idx) in question.references" :key="idx">
                        <div class="flex items-center space-x-3">
                            <input type="text" :name="'references['+idx+'][title]'" x-model="ref.title" placeholder="Reference Title (e.g. MS Docs)"
                                   class="w-1/2 border-gray-300 rounded text-sm px-3 py-1.5 focus:border-cyan focus:ring-cyan">
                            <input type="text" :name="'references['+idx+'][url]'" x-model="ref.url" placeholder="URL (e.g. https://...)"
                                   class="w-1/2 border-gray-300 rounded text-sm px-3 py-1.5 focus:border-cyan focus:ring-cyan">
            ...
            <div class="border-b border-gray-150 pb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label for="media_file" class="block text-xs font-bold text-gray-400 uppercase mb-2">Upload Image File</label>
                    ...
                    <div>
                        <label for="media_caption" class="block text-xs font-bold text-gray-400 uppercase mb-2">Exhibit Caption</label>
                        <input type="text" name="media_caption" id="media_caption" placeholder="e.g. Network topology diagram"
                               class="w-full border-gray-300 rounded text-sm px-3 py-2 focus:border-cyan focus:ring-cyan">
                    </div>
                </div>
            </div>
            ...
            <div class="pt-4 flex justify-end space-x-3">
                <a href="{{ route('admin.questions.index', ['exam_id' => request('exam_id')]) }}" class="bg-gray-100 hover:bg-gray-200 text-gray-600 text-xs font-bold py-2.5 px-6 rounded transition">
                    Cancel
                </a>
                <button type="submit" :disabled="saving"
                    ...
                    <span x-text="saving ? 'Saving...' : 'Save Question'"></span>
                </button>
            </div>
        </form>
    </div>
    ...
    return {
        question: {
            ...
            question_data: {
                ...
                boxes: [
                    { id: 'box_1', label: 'Box 1', optionsText: '', options: [], correct_answer: '' }
                ],
                matching_pairs: [
                    { left: '', right: '' }
                ]
