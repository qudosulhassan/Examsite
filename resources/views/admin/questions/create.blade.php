...
<div class="max-w-5xl mx-auto space-y-8 pb-24"
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-gray-200">
        <div>
            ...
            <p class="text-xs text-gray-500 mt-0.5">Create and configure an exam question with options and correct answer grading rules.</p>
        </div>
    ...
    <form action="{{ route('admin.questions.store') }}" method="POST" enctype="multipart/form-data" @submit="submitForm($event)" class="space-y-8">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <textarea name="question_text" id="question_text" rows="5" x-model="question.question_text" required
                              placeholder="Enter the main question text (Markdown, code blocks, and HTML supported)..."
                ...
                <div class="bg-gray-50/70 p-4 border border-gray-200 rounded-lg">
                    <label for="question_image" class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1.5">Question Image / Diagram (Optional)</label>
                    <input type="file" name="question_image" id="question_image" accept=".png,.jpg,.jpeg,.webp"
                           class="w-full border border-gray-300 rounded-lg text-sm px-3 py-2 focus:border-cyan focus:ring-cyan file:mr-4 file:py-1 file:px-3 file:rounded-md file:border-0 file:text-xs file:font-semibold file:bg-navy file:text-white hover:file:bg-opacity-90">
                    <p class="text-[11px] text-gray-500 mt-1">Upload a network diagram, topology image, or architectural graphic to display in the question prompt.</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white border border-gray-200 rounded-xl p-6 shadow-sm space-y-4">
            ...
            <div>
                ...
                <input type="text" name="instructions" id="instructions" x-model="question.instructions" placeholder="e.g. Select one answer. Each correct choice is worth one point."
    ...
    return {
        ...
        question: {
            ...
            question_data: {
                ...
                boxes: [
                    { id: 'box_1', label: 'Box 1', optionsText: 'Option 1, Option 2', options: ['Option 1', 'Option 2'], correct_answer: 'Option 1' }
                ],
                matching_pairs: [
                    { left: '', right: '' }
                ]
