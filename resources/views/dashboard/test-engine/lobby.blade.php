...
@section('content')
<div class="relative bg-gradient-to-br from-[#07101E] via-navy to-[#0F172A] rounded-3xl overflow-hidden py-12 px-4 sm:px-6 shadow-2xl">
    ...
    <div class="relative z-10 max-w-4xl mx-auto space-y-8">
        ...
        <nav class="flex text-[11px] font-bold uppercase tracking-widest text-gray-400 space-x-3 mb-8">
            ...
            <span class="text-gray-600">/</span>
            <span class="text-cyan">{{ $exam->exam_code }} Lobby</span>
        </nav>
        ...
        <div class="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-8 md:p-12 shadow-[0_20px_50px_rgba(0,0,0,0.3)] space-y-10">
            ...
            <form action="{{ route('dashboard.test-engine.start', $exam->id) }}" method="POST" class="space-y-10">
                @csrf
                ...
                <div class="pt-8 border-t border-white/10 flex flex-col md:flex-row justify-between items-center gap-6">
                    <a href="{{ route('dashboard.test-engine') }}" class="text-[11px] font-bold text-gray-400 hover:text-cyan transition-colors uppercase tracking-widest">
